...
def test(data_loader, model, img_names, sets):
    ...
    for batch_id, batch_data in enumerate(data_loader):
        # forward
        volume,mask_array,ori_mask = batch_data
        if not sets.no_cuda:
            volume = volume.cuda()
        with torch.no_grad():
            probs = model(volume)
...
if __name__ == '__main__':
    ...
    # testing
    img_names = [info.split("|")[0] for info in load_lines(dataset_info["fp_val"])]
    masks = test(data_loader, net, img_names, sets)
    
    # evaluation: calculate dice 
    label_names = [info.split("|")[1] for info in load_lines(dataset_info["fp_val"])]
